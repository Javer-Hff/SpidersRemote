...
s1 = "https://wirelesspub-global.xiaozhu.com/app/xzfd/web/500/geo/basic"
s2 = "https://wirelesspub-global.xiaozhu.com/app/xzfd/web/500/geo/basic?clientFrom=web&clientVersion=500&gatets=1719305155&geoCode=0&items=0&level=2&role=xzfd&type=0"
gatesign = "(s2HamcSHA1,key:(s1md5))"
gatets = "time"

url1 = "https://bizguard.xiaozhu.com/geetest/register?clientFrom=web&clientVersion=500&role=xzfd&gatets=1719303259&scene=login&client=PC&gatesign=da5151761eaece2170401dc57de0945f4f685897"
...
if __name__ == '__main__':
    ...
    # step1Json = step1_for_proxyweb(url+"/auth/register")
    # 根据第一个请求的返回值GT请求第二个链接(请求没啥用) 请求了一些js文件
    step2Response = step2(step1Json['content']["gt"])
    step2Json = handle_response_to_json(step2Response)
    # 根据第一个请求的返回值GT和challenge请求第三个链接拿到 C 和 S，好像也没啥用 s后期也会变
    step3Response = step3(step1Json['content']["gt"], step1Json['content']["challenge"])
    step3Json = handle_response_to_json(step3Response)
    # 根据第一个请求的返回值GT和challenge请求第四个链接，好像也没啥用
    step4Response = step4(step1Json['content']["gt"], step1Json['content']["challenge"])
    step4Json = handle_response_to_json(step4Response)
    # 根据第一个请求的返回值GT和challenge请求第五个链接，拿到图片信息 和 新的challenge（比原先的challenge多2位）
    step5Response = step5(step1Json['content']["gt"], step1Json['content']["challenge"])
    step5Json = handle_response_to_json(step5Response)
    pprint(step5Json)
    # 获取极验静态地址的前缀，用来拼接下载图片的
    static_servers_prefix_url ="https://" + step3Json["data"]["static_servers"][0] + "/"
